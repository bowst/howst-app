...
var machineInfo = exports.getMachineInfo = function(hostname, options){
  var howst = utils.loadHowstFile("howst");
  var machine =  utils.getMachineByHostname(hostname, howst);
  var table = new Table({ chars: {'mid': '', 'left-mid': '', 'mid-mid': '', 'right-mid': ''}, colAligns:['right'] });
  for(var key in machine){
    table.push([key.bold, machine[key]]);
  }
  console.log(table.toString());
};