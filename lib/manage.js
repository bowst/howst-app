  ...
  if(utils.getMachineByHostname(hostname, howst)){
    ...
    utils.configMachine(newMachine, function(configuredMachine){
      ...
      if(!utils.saveHowstFile("howst", howst)){
        ...
        if(!configuredMachine.is_existing_site){
          console.log("Here is some settings information you'll need for Drupal's install script: ");
          console.log("Database Name: %s", configuredMachine.db_name);
          console.log("Database User: %s", configuredMachine.db_user);
          console.log("Database Host: %s", configuredMachine.db_host);
          console.log("Database Password: %s", configuredMachine.db_pass);
        }
        console.log("Happy Drupaling!".bold);
      }
    })
  }
};
